...
class TaskState(BaseModel):
    ...
    llm_call: LLMCallState = Field(default_factory=LLMCallState)
...
def run_pipeline(prompt: str, workspace: Path, project_context: dict | None = None, task_id: str | None = None) -> TaskState:
    ...
    save_state(workspace, state)
    if not pre_report.is_valid or not state.boris_constraints_report.action_allowed:
        reason = "; ".join(state.boris_constraints_report.stop_conditions) or "; ".join(pre_report.reasoning) or "Pre-LLM gate blocked execution."
        state.llm_call = LLMCallState(attempted=False, skipped_reason=reason)
        state.llm_response = None
        state.execution_trace = ExecutionTrace(executed=False, actions=["llm_skipped", "execution_skipped"], result=clarification_question(state))
    ...
    state.status = "llm"
    state.llm_call = LLMCallState(attempted=True, model=model_name())
    ...
    state.llm_call.provider = state.llm_response.provider
...
def clarification_question(state: TaskState) -> str:
    missing = state.sima_output.missing_intent_fields if state.sima_output else []
    intent_type = state.sima_output.intent_type if state.sima_output else "unknown"
    if state.sima_output and state.sima_output.referent_unresolved:
        return "Уточни, пожалуйста, что именно ты имеешь в виду: какой файл, объект или предыдущее действие нужно использовать?"
    if intent_type == "restore":
        return "Что именно восстановить и из какого источника: файл, версию, backup или предыдущее изменение?"
    if missing:
        return "Не хватает данных для безопасного действия: " + ", ".join(missing) + ". Уточни задачу, пожалуйста."
    return "Уточни задачу, пожалуйста: что именно нужно сделать и с каким объектом?"
